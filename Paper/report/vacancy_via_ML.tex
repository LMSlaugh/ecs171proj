...
Another, less commonly encountered method uses specially-programmed cameras that sense presence through various algorithms such as depth analysis \cite{Peterson} or facial recognition \cite{Viola}. These methods have the benefit of providing an integer count of occupants, making them a great fit for predictive model control. However, these systems have high capital and maintenance costs and the overall system expenditure is not likely to outweigh the possible energy savings produced by vacancy turndowns.
...
\begin{table}[H]
        ...
        \begin{tabularx}{0.5\textwidth}{CC}
                ...
                \\ \\ \\ \cmidrule(r){1-1} \cmidrule(r){2-2}
                \multirowcell{2}{Occupied}                    & \multirowcell{2}{1 indicates occuiped\\0 indicates vacant}
                \\ \\ \bottomrule
        \end{tabularx}
\end{table}
